...
-   Q1. Consider the use case of `Compute basic/advance result`, **HOW** is this functionality made available to both JiBaBoom and Adminstrator _(Hint: They are different)_? `( for JiBaBoom, can only insert basic and advance data, compute basic and advance data but cannot view basic and advance data. where as for administrator they can view and insert basic and advance data but cannot compute.)`
-   Q2. Consider the use case of `Insert basic/advance data`. **HOW** is the data stored? `(The data is stored by a JSON package which is then stringify to human readable form shown in the table dataviewer and resultviewer. Insert is writing a JSON package and then sent by a function ( API) for the database to insert.)`
...
3. How should the frontend display data from the database? `The frontend displays data through communicating the backend which is the middleman, where the backend talks to the database. `
4. How does the frontend communicate with backend? `The frontend talks to the backend through the use of REST API (HTTP) `
...
```
[The frontend gets data through exporting the functions that carries the sql query as well as connecting to the database; and then creating the API with the exported functions and getting the data from the frontend. The frontend then retrieve the data from the input value of the form (if using insert) or displays the results ( GET response) through the use of calling a javascript file that contains functions that call the API, created in the backend, to map the input value and send to the backend via JSON format.]
```
